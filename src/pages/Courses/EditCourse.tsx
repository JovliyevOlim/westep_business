...
import {ArrowLeft, Camera, CheckCircle2, LoaderCircle, Save} from "lucide-react";
...
export default function EditCourse() {
    ...
    const courseQuery = useGetCourseById(id);
    ...
    const formik = useFormik<CourseFormValues>({
        initialValues: {
            name: course?.name || "",
            primaryCategoryId: course?.primaryCategoryId || "",
            subcategoryId: course?.subcategoryId || "",
            description: course?.description || "",
            fullDescription: course?.fullDescription || "",
            skillTagIds: course?.skillTagIds || [],
        ...
        onSubmit: async () => {
            ...
            try {
                let attachmentId: string | null = formik.values.attachmentId || course?.attachmentId || null;
                ...
                try {
                    ...
                    if (isUnauthorizedError(error)) {
                        const message = getSessionErrorMessage();
                        formik.setStatus(message);
                        showErrorToast(new Error(message), "Kursni tahrirlab bo'lmadi");
                        navigate("/login", {replace: true});
                        return;
                    }
                    throw parseApiError(error, "Kursni tahrirlab bo'lmadi.");
                }

                showSuccessToast("Kurs ma'lumotlari yangilandi");
                navigate("/courses");
            } catch (error) {
                const parsedError = parseApiError(error, "Kursni tahrirlashda xatolik yuz berdi");
    ...
    const isSubmitting = isUpdating || isUploadingFile;

    if (courseQuery.isLoading) {
        return (
            <div className="mx-auto flex max-w-[1180px] items-center gap-3 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-950">
                <LoaderCircle className="h-5 w-5 animate-spin text-blue-600" />
                <span className="text-sm font-medium text-slate-600 dark:text-slate-300">Kurs ma'lumotlari yuklanmoqda...</span>
            </div>
        );
    }
    ...
    if (!course) {
        return (
            <div className="mx-auto max-w-[720px] rounded-2xl border border-slate-200 bg-white p-8 text-center shadow-sm dark:border-slate-800 dark:bg-slate-950">
                <h1 className="text-xl font-semibold text-slate-950 dark:text-slate-100">Kurs topilmadi</h1>
                <Button type="button" onClick={() => navigate("/courses")} className="mt-5 rounded-xl bg-blue-600 text-white hover:bg-blue-700">
                    Kurslarga qaytish
                </Button>
            </div>
        );
    ...
    return (
        <div className="mx-auto max-w-[1180px] space-y-4 pb-10">
            <PageMeta title="Kursni tahrirlash" description="Kurs ma'lumotlarini tahrirlash sahifasi" />
            ...
            <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-950">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h1 className="text-2xl font-semibold text-slate-950 dark:text-slate-100">Kursni tahrirlash</h1>
                        <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">Kurs ma'lumotlarini shu sahifaning o‘zida yangilang.</p>
                    </div>
                </div>
            </section>
            ...
            <form onSubmit={formik.handleSubmit} className="space-y-4">
                    ...
                    <div className="space-y-4">
                        <div className={formSectionClass}>
                            ...
                            <div className="mt-3 overflow-hidden rounded-2xl border border-slate-200 bg-slate-50 dark:border-slate-800 dark:bg-slate-900">
                                {currentImageUrl ? (
                                    <img src={currentImageUrl} alt="Course preview" className="h-48 w-full object-cover" />
                                ) : (
                                    ...
                                    return (
                                        ...
                                        >
                                            <input
                                                ...
                                                onChange={() => {
                                                    const next = checked
                                                        ? formik.values.skillTagIds.filter((skillId) => skillId !== skill.id)
                ...
                <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-950">
                    <div className="flex items-start gap-3">
                        <div className="rounded-xl bg-emerald-50 p-2 text-emerald-700 dark:bg-emerald-500/10 dark:text-emerald-200">
                            <CheckCircle2 className="h-5 w-5" />
                        </div>
                        <div>
                            <h3 className="text-sm font-semibold text-slate-950 dark:text-slate-100">{formik.values.name || "Kurs nomi"}</h3>
                            <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">{formik.values.description || "Hali tavsif kiritilmagan."}</p>
                        </div>
                    </div>
                </div>
            </form>
